    ...
    */
    ...
    checkCollisions() {
        ...
        this.stones.forEach((stone, index) => {
         ...
         // 4. Kollision Wurfgeschosse mit Gegnern 
        //this.throwableObjects.forEach((stone, stoneIndex) => {
        //    this.enemies.forEach((enemy, enemyIndex) => {
        //        // Prüfen, ob Stein und Gegner existieren und kollidieren
        //        if (stone && enemy && !enemy.isDead && stone.isColliding(enemy)) { // Prüfe, ob Gegner noch "lebt"
        //            enemy.hit();                        // Gegner benachrichtigen, dass er getroffen wurde
        //            if(typeof stone.destroy === 'function') {
        //               stone.destroy();
        //            }
        //            this.throwableObjects.splice(stoneIndex, 1);
        ...
        if (this.treasureChest && this.character.isColliding(this.treasureChest)) {
         ...
         this.throwableObjects = this.throwableObjects.filter(stone => 
            ...
            //const isOffScreen = stone.x + stone.width /*> 0 && stone.x < */ < -this.camera_x || stone.x > this.canvas.width - this.camera_x; // Grober Bereich
            //if (isOffScreen && typeof stone.destroy === 'function') {
            //    stone.destroy(); // Zerstöre das Objekt, wenn es eine destroy-Methode hat
            //}
         //   return !isOffScreen && stone.y < 450; // Nur Steine behalten, die sichtbar und nicht zu tief sind
     //   });
